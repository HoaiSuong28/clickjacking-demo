...
const SecurityDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchRecentAttacks = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/admin/security/recent-attacks', {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
        });
  ...
  useEffect(() => {
    ...
    const fetchLogFiles = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/admin/security/logs', {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
        });
  ...
  const viewLogFile = async (filename) => {
    ...
    setError(null);
    try {
      const response = await axios.get(`http://localhost:5000/api/admin/security/logs/${filename}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        params: { limit: 100 }
      });
  ...
  const clearBlacklist = async () => {
    ...
    try {
      await axios.post('http://localhost:5000/api/admin/security/clear-blacklist', {}, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
